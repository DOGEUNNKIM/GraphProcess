...
int main(int  argc, char **argv) {
  ...
  NodeData *nodes = (NodeData *)malloc((int)A.num_nodes * sizeof(NodeData));
...
////
//
    int *frontier1 = (int *)malloc(A.num_nodes * sizeof(int));
...
////
  ...
  int row_ptr_process_bytes = sizeof(int ) * (A.num_nodes  + 1);
  int col_idx_process_bytes = sizeof(int ) * (A.num_edges+1);
  int node_bytes = sizeof(int) * A.num_nodes;
  ...
  std::fill(frontier_map, frontier_map + A.num_nodes,  (int)-1);
  std::fill(Vprop_map, Vprop_map + A.num_nodes, (int)-1);
  ...
  for (int i = 0; i < (int)(A.num_nodes + 1); ++i) {
