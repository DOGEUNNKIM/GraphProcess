    ...
    for (beforeIDX = 0; beforeIDX < _NUM_NODES_; beforeIDX = currIDX) {
        ...
        for (int tile = 0; tile < NUM_TILES; tile++) {
            ...
            for (int idx = beforeIDX; idx < currIDX; idx++) {
                ...
                int row_ptr_start = row[(tile*_NUM_NODES_+ old_Vprop_idx) >> 4].data[(tile*_NUM_NODES_+ old_Vprop_idx)%16];
                int row_ptr_end   = row[(tile*_NUM_NODES_+ old_Vprop_idx + 1) >> 4].data[(tile*_NUM_NODES_+ old_Vprop_idx + 1)%16];
...
//
  ...
  //}
  //printf("col i =%ld\n",i);
  //for (i = 0; i < row_ptr_process_bytes/4; ++i) {
...
//
  ...
  //  }
  //}
  //for (size_t i = 0; i < row_ptr_process_bytes/4; ++i) {
  //  if(row_map[i] != T.row_ptr[i / VDATA_SIZE].data[i % VDATA_SIZE]){
  //      printf("row\n");
  //  }
  //}
  printf("frontier1, %ld\n",A.num_nodes * sizeof(int));
  ...
  //}
  //for (int i = 0; i < 100; ++i) {
  //      printf("frontier[%d] = %d,frontier[%d] = %d\n"
  //      ,i, frontier_map[i]
  //      ,i, frontier1[i]);
  //}
  for (int i = 0; i < A.num_nodes; i++) {
