...
void RecordActiveNode(const pvector<NodeID> &activelist, int iter){
  ...
  string file_name = output_directory + "/sssp_push/" + graph_name + "_iter" + to_string(iter) + ".txt";
...
int main(int argc, char* argv[]) {
  ...
  auto SSSPVCPM = [&sp, &cli] (const WGraph &g){
    return SSSP_push(g, sp.PickNext(), cli.delta());
  };

  // BenchmarkKernel(cli, g, SSSPBound, PrintSSSPStats, VerifierBound);
  BenchmarkKernel(cli, g, SSSPVCPM, PrintSSSPStats, VerifierBound);
  return 0;
}
