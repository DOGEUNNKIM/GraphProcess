...
class CLBase {
 protected:
  ...
  char** argv_;
  std::string name_;
  std::string get_args_ = "f:g:o:hk:su:m:c:w";
 ...
 public:
  CLBase(int argc, char** argv, std::string name = "") :
    ...
    AddHelpLine('c', "", "Do Graph Tiling for COLLECT", "false");
    AddHelpLine('o', "directory", "output directory path");
  }
  ...
  void virtual HandleArg(signed char opt, char* opt_arg) {
    switch (opt) {
      ...
      case 'c' : tiling_ = true;                            break;
      case 'o': output_dir = std::string(opt_arg);          break;
      case 'w': weight_trace = true;
    }
  }
