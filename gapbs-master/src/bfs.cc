...
void RecordActiveNode(const SlidingQueue<NodeID> &queue, int iter){
  if(output_directory == "")
    return;
...
void RecordUnvisitedNode(const pvector<NodeID> &parent, int iter){

  string file_name = "data/soc-orkut_iter" + to_string(iter) + ".txt";
  cout << "Save Unvisited Node [iter" << iter << "]" << endl;
  ofstream o(file_name);
  for(NodeID u=0; u < int(parent.size()); u++){
    if(parent[u] < 0){
      o << u << endl;
    }
  }
