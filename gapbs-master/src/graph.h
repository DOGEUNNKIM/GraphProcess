...
class CSRGraph {
 ...
 public:
  CSRGraph() : directed_(false), num_nodes_(-1), num_edges_(-1), num_tiles_(1),
  ...
  CSRGraph(CSRGraph&& other) : directed_(other.directed_),
    num_nodes_(other.num_nodes_), num_edges_(other.num_edges_), num_tiles_(other.num_tiles_),
  ...
  void set_num_tiles(int64_t num_tiles){
  ...
  static DestID_** GenIndex(const pvector<SGOffset> &offsets, DestID_* neighs) {
    NodeID_ length = offsets.size(); // # of vertex 
    DestID_** index = new DestID_*[length];
    #pragma omp parallel for
    for (NodeID_ n=0; n < length; n++)
      index[n] = neighs + offsets[n]; // @s : neighs is the start pointer of neighs[]
    return index;
  }
