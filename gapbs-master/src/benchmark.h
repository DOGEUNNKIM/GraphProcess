...
void BenchmarkKernel(const CLApp &cli, const GraphT_ &g,
  ...
  for (int iter=0; iter < cli.num_trials(); iter++) {
    trial_timer.Start();
    trial_timer.StartCycle();
    auto result = kernel(g);
    trial_timer.StopCycle();
    trial_timer.Stop();
    printf("CPU Cycles : %lld\n", trial_timer.Cycle());
    PrintTime("Trial Time", trial_timer.Seconds());
    total_cycles += trial_timer.Cycle();
  ...
  printf("Total CPU Cycles : %lld,  Average CPU Cycles : %lld\n", total_cycles, total_cycles / cli.num_trials());
