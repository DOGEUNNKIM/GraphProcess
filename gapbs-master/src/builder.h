...
class BuilderBase {
  ...
  bool symmetrize_;
  bool needs_weights_;
  bool in_place_ = false; //@s : true when -m
  int64_t num_nodes_ = -1;
  
  bool tiling_ = false;
  int64_t delim = 7 + 2 + 2 + 3; //@s : hard coding here
  int64_t num_tiles_ = 1;
  int64_t unit_v = -1;

 public:
  explicit BuilderBase(const CLBase &cli) : cli_(cli) {
    symmetrize_ = cli_.symmetrize();
    needs_weights_ = !std::is_same<NodeID_, DestID_>::value;  //@s : pagerank -> False
  ...
  pvector<NodeID_> CountDegrees(const EdgeList &el, bool transpose) {
    if(tiling_ && transpose){
      if(symmetrize_){
        cout << "Tiling doesn;t support symmetrize" << endl;
        exit(-30);
      }
      num_tiles_ = ceil((float)num_nodes_ / pow(2, delim));
      unit_v = pow(2, delim); 
      cout << "Necessary Tile # for COLLECT : " << num_tiles_ << endl;
      pvector<NodeID_> degrees(num_nodes_ * num_tiles_, 0);
      #pragma omp parallel for
      for (auto it = el.begin(); it < el.end(); it++) {
        Edge e = *it; 
        int64_t t_idx = (int64_t)e.u / unit_v; // dst->src tiling
        // int64_t t_idx = (int64_t)e.v / unit_v; //src->dst tiling
        for(int t = 0; t < num_tiles_; t++){
          if(t_idx < (t+1)){
            fetch_and_add(degrees[(NodeID_) e.v + t*num_nodes_], 1); // dst->src tiling
            //degrees[e.u + t*num_nodes_] += 1; // src->dst tiling
            break;
          }
        }              
      }
  ...
  pvector<SGOffset> ParallelPrefixSum(const pvector<NodeID_> &degrees) {
    const size_t block_size = 1<<20;    //1024*1024
    const size_t num_blocks = (degrees.size() + block_size - 1) / block_size; // 1 + nodes/block_size
    pvector<SGOffset> local_sums(num_blocks);
    #pragma omp parallel for
    for (size_t block=0; block < num_blocks; block++) { //block 개수 만큼
      SGOffset lsum = 0;
      size_t block_end = std::min((block + 1) * block_size, degrees.size()); // 각 block의 마지막 
      for (size_t i=block * block_size; i < block_end; i++) //block의 시작부터 끝까지
        lsum += degrees[i];
      local_sums[block] = lsum; //local_sums contains  edge # per block
    }
    ...
    bulk_prefix[num_blocks] = total; // block 별 prefix sum
    ...
    return prefix; //@s : prefix vector is the CSR Row
  }
  ...
  */
  void MakeCSR(const EdgeList &el, bool transpose, DestID_*** index,
    ...
    pvector<SGOffset> offsets = ParallelPrefixSum(degrees);  //@s : offsets = CSR row

    cout << "num_nodes, offsets size, offsets[-1] : " << num_nodes_ << ", " << offsets.size() << ", " << offsets[num_tiles_ * num_nodes_] << endl;

    *neighs = new DestID_[offsets[num_tiles_ * num_nodes_]];  //@s : neighs size = # of edges
    *index = CSRGraph<NodeID_, DestID_>::GenIndex(offsets, *neighs); //make edges spaces for each vertices 
    //index size == offsets size (num_nodes * num_tiles + 1)         // *index[n] == n'th vertex's edge starts array pointer
    
    // Fill the CSR Space with Edges (CSR Column)
    // Tiling ver
    if(tiling_ & transpose){
      #pragma omp parallel for
      for (auto it = el.begin(); it < el.end(); it++) {
        ...
        for(int t = 0; t < num_tiles_; t++){
          if(t_idx < (t+1)){
            (*neighs)[fetch_and_add(offsets[t*num_nodes_ + static_cast<NodeID_>(e.v)], 1)] = GetSource(e); // dst->src tiling
            //(*neighs)[fetch_and_add(offsets[t*num_nodes_ + static_cast<NodeID_>(e.u)], 1)] = GetSource(e); // src->dst tiling
            break;
          }
        }              
      }
    ...
    cout << "transpose : " << transpose << " symmetrize : " << symmetrize_ << endl;
  ...
  CSRGraph<NodeID_, DestID_, invert> MakeGraphFromEL(EdgeList &el) {
    ...
    if (needs_weights_) //pr default false
      Generator<NodeID_, DestID_, WeightT_>::InsertWeights(el);
    if (in_place_) {    //pr default false
      MakeCSRInPlace(el, &index, &neighs, &inv_index, &inv_neighs);
    } 
    else {
      MakeCSR(el, false, &index, &neighs);
      if (!symmetrize_ && invert) {
        cout << "Make Invert CSR" << endl;
  ...
  CSRGraph<NodeID_, DestID_, invert> MakeGraph() {
    ...
    g.set_num_tiles(num_tiles_);
    
    // if(tiling_){
    //   ofstream o("graph_tiled.txt");
  	//   o << "[EdgeList for OUTGOING Edges]" << endl;
    //   for(NodeID_ u=0; u < g.num_nodes();u++)
    //     for(NodeID_ v : g.out_neigh(u))
    //       o << u << " " << v << endl;
    //   o << "====================" << endl;
    //   o << "[EdgeList for INCOMING Edges (Do Tiling)]" << endl;
    //   for(NodeID_ u=0; u < g.num_tiles() * g.num_nodes();u++)
    //     for(NodeID_ v : g.in_neigh(u))
    //       o << u%g.num_nodes() << " " << v << endl;
    //   // cout << "break point" << endl;
    //   // exit(-30);
    // }

    if (in_place_ || tiling_)
      return g;
    else
