...
pvector<ScoreT> PageRankPullGS(const Graph &g, int max_iters,
  ...
  pvector<ScoreT> old_score(g.num_nodes(), 0); 

  #pragma omp parallel for
  for (NodeID n=0; n < g.num_nodes(); n++)
    outgoing_contrib[n] = init_score / g.out_degree(n); // initalize
  for (int iter=0; iter < max_iters; iter++) { 
    double error = 0;
    for(NodeID t=0;t<g.num_tiles();t++){
      // #pragma omp parallel for reduction(+ : error) schedule(dynamic, 16384)
      for (NodeID u=0; u < g.num_nodes(); u++) {
        ScoreT incoming_total = 0;
        for (NodeID v : g.in_neigh(u + t*g.num_nodes())){
          incoming_total += outgoing_contrib[v];
        } 
        
        if(t == g.num_tiles()-1){ // last tile
          scores[u] = base_score + kDamp * (scores[u] + incoming_total);
          error += fabs(scores[u] - old_score[u]);
          outgoing_contrib[u] = scores[u] / g.out_degree(u); 
        }
        else if(t == 0){ // first tile
          old_score[u] = scores[u];
