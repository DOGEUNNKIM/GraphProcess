...
int main(int argc, char **argv) {
  ...
  auto bo_iter = xrt::bo(device, 4, krnl.group_id(5));
  ...
  auto bo_iter_map = bo_iter.map<int *>();
  ...
  std::fill(bo_iter_map, bo_iter_map + 1, 666);
  ...
  bo_iter.sync(XCL_BO_SYNC_BO_TO_DEVICE);
  ...
  auto run = krnl(bo0, bo1, bo2,bo_score_1, bo_score_2, bo_iter);
  run.wait();
  ...
  bo_iter.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
  ...
  std::cout << "FPGA iter: " << bo_iter_map[0] << std::endl;
